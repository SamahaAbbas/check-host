...
def ping_data_parser(result_data, data_frame_temp, id_key):
    timeout_flag = False  # Initialize flag to False
        ...
        if country.lower() == "iran":  # Check if the country is Iran
            if result_data[nod_location] != None:
                ...
                if data_in_result_data != None:
                    ...
                    data_frame_temp.loc[count] = [f"{country}, {city}", f"{time_1}/{time_2}/{time_3}/{time_4} ms.",
                                                   f"{code_1}/{code_2}/{code_3}/{code_4}", ip_address]
                    if "TIMEOUT" in [code_1, code_2, code_3, code_4]:
                        timeout_flag = True  # Set flag to True if TIMEOUT occurs
    # remove index // set index to ""
    data_frame_temp.index = [""] * len(data_frame_temp)
    return data_frame_temp, timeout_flag  # Return the flag along with the dataframe
...
def ping_part(args):
    ...
    if timeout_flag:
        ...
        return ip_address, "\033[92mGOOD\033[0m"  # Colored green for GOOD
